...
export default function ShipmentRegisterLayout({ children }: ShipmentRegisterLayoutProps) {
	...
	const [domicilioOrigenLoading, setDomicilioOrigenLoading] = useState(true);

	useEffect(() => {
		if (!userPayload) return;
		DomiciliosService.getDomicilioByClienteId(userPayload.id)
			.then((userDomicilio) => {
				...
				const origenCliente = {
					...
					localidadID: userDomicilio.localidad.id,
				};
				setShipment({
					...shipment,
					cliente: userPayload.id,
			...
			.finally(() => {
				setDomicilioOrigenLoading(false);
			});
	...
	if (isCreated)
		return (
			<ShipmentCreatedConfirmation nroSeguimiento={nroSeguimiento} handleGoBack={handleGoBack} />
		);
					...
					<div className='rounded-md h-full flex flex-col'>
						...
						<div className='bg-white shadow-lg space-y-10 px-8 py-6 flex-1 rounded-b-md'>
							{domicilioOrigenLoading ? (
								...
								<Accordion>
									...
									<AccordionDetails className='flex flex-col gap-8'>
										<FormControl>
											<InputLabel htmlFor='calle' shrink={true}>
												Calle
											</InputLabel>
											<Input
												id='calle'
										...
										<FormControl>
											<InputLabel htmlFor='numero' shrink={true}>
												Número
											</InputLabel>
											<Input
												id='numero'
										...
										<FormControl>
											<InputLabel htmlFor='depto' shrink={true}>
										...
										<FormControl>
											<InputLabel htmlFor='piso' shrink={true}>
												Piso (opcional)
											</InputLabel>
											<Input
												id='piso'
										...
										<FormControl>
											<InputLabel htmlFor='descripcion' shrink={true}>
							...
							<div className='flex flex-col gap-3'>
								...
								<p className='text-gray-500'>{userPayload.email}</p>
							</div>
